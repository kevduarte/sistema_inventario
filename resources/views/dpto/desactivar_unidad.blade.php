...
<h2 style= "font-family: 'initial';">Desactivar del registro la unidadd: {{$unidad->codigo_unidad}} </h2>
    ...
    <form method="POST" action="{{ route('baja_unidad_prueba') }}" onsubmit="return confirmation()">
      ...
      <div class="form-row">
       ...
       id="codigo_unidad" name="codigo_unidad">
        ...
        <div class="form-group col-md-4">
          <label for="codigo_unidad">Número de serie</label>
          <input type="text"  value="{{$unidad->num_serie}}" disabled  class="form-control @error('codigo_unidad') is-invalid @enderror"
          id="codigo_unidad" name="codigo_unidad">
          @error('codigo_unidad')
          <span class="invalid-feedback" role="alert">
           <strong>{{ $message }}</strong>
         ...
         <input type="text"  value="{{$unidad->num_serie}}" hidden class="form-control @error('codigo_unidad') is-invalid @enderror"
       id="serie" name="serie">
