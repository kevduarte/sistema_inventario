...
<td>{!! $encargados->estado !!}</td>
    ...
    <td><a style="color: red;" href="{{ route('desactivar_jefelab_dpto',$encargados->id_user)}}"><?php if($encargados->estado=='actual'){echo "Desactivar";}?></a></td>
    <td><a style="color: red;" href="{{ route('activar_jefelab_dpto',$encargados->id_user)}}"><?php if($encargados->estado=='anterior'){echo "Activar";}?></a></td>
    <td><a  style="color: green;" href="{{ route('restablecer_contra_dpto',$encargados->id_user)}}" ><?php if($encargados->estado=='actual'){echo "Restablecer contraseña";}?></a></td>
  </tr>
  @endforeach
</tbody>
